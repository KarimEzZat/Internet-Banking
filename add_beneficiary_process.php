...
<?php
                ...
                {
                echo '<script>alert("لا تستطيع اضافه نفسك  كمستفيد");';
                     echo 'window.location= "add_beneficiary.php";</script>';
                }
                
                elseif($s1==$sender_id && $s2==$acc_no)
                {
                    echo '<script>alert("هذا المستفيد موجود بالفعل");';
                     echo 'window.location= "add_beneficiary.php";</script>';
                }
                
                elseif($rws[1]!=$Payee_name && $rws[0]!=$acc_no && $rws[11]!=$branch && $rws[12]!=$ifsc)
                {
                echo '<script>alert("المستفيد غير موجود من فضلك ادخل بيانات صحيحه");';
                     echo 'window.location= "add_beneficiary.php";</script>';
                }
